...
import { 
  User, Key, Link, Bell, CreditCard, ShieldCheck, Languages, 
  HelpCircle, Trash2, ChevronRight, Edit3, LogOut, AlignLeft, 
  Calendar, Heart, FileText, Smartphone, Moon, Image as ImageIcon
} from "lucide-react";
...
const Settings = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      <h1 className="text-2xl font-serif text-amber-900 font-bold mb-8">Settings</h1>
...
const SettingsCard = ({ title, icon, description, toggle = false, danger = false }) => {
  const [isEnabled, setIsEnabled] = useState(false);
  
  return (
    <div className={`p-4 border rounded-lg ${danger ? 'border-red-100' : 'border-amber-100'} hover:shadow-sm transition-shadow`}>
      <div className="flex items-start justify-between">
        ...
        {toggle ? (
          ...
          <button className={`p-2 rounded-full hover:bg-amber-50 transition-colors ${danger ? 'text-red-500' : 'text-amber-700'}`}>
